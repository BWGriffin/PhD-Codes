...
graph.loop <- function(sc, j, df, mass, mass2, ti, vary, l1, l2)
  {
##Load and Setup##
  dirname <- "MM/"
  ...
  # read in raw data
  mm_bu   <- read.csv((paste0(buname2,"_MC_mean.csv")), header = TRUE, sep = ",", quote =
                        "\"",dec = ".", fill = TRUE, comment.char = "")
  mm_bi   <- read.csv((paste0(biname2,"_MC_mean.csv")), header = TRUE, sep = ",", quote =
                        "\"",dec = ".", fill = TRUE, comment.char = "")
  mm_sdbu <- read.csv((paste0(buname2,"_MC_stdev.csv")), header = TRUE, sep = ",", quote =
                       "\"",dec = ".", fill = TRUE, comment.char = "")
  mm_sdbi <- read.csv((paste0(biname2,"_MC_stdev.csv")), header = TRUE, sep = ",", quote =
                         "\"",dec = ".", fill = TRUE, comment.char = "")
...
## e.g. to change from Knee to the Shoulder use ctrl+f and replace Knee with Shoulder do not use " or "_ just use the word

graph.loop (sc = "0_73/", j = "Shoulder", df = "_ABAD", mass = "_MM1_m1", mass2 = "_MM1_md1", ti= "Shoulder Abduction", 
            vary =" average type",l1 = 'Mean', l2 = 'Median')
